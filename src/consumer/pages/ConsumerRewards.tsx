          ...
          return (
            ...
            >
              <div className="flex justify-between items-start mb-6">
                <div className={`w-14 h-14 rounded-full flex items-center justify-center transition-colors ${canAfford ? 'bg-gray-50 text-gray-700 group-hover:bg-gray-800 group-hover:text-white' : 'bg-gray-50 text-gray-400'}`}>
                  <Icon size={24} strokeWidth={1.5} />
                </div>
                {canAfford && (
                  <button className="w-8 h-8 rounded-full border border-gray-100 flex items-center justify-center text-gray-400 group-hover:border-gray-800 group-hover:bg-gray-800 group-hover:text-white transition-all">
                    <ArrowUpRight size={14} strokeWidth={1.5} />
                  </button>
                )}
              </div>
              <h3 className="text-xl font-medium text-gray-700 mb-3">{reward.title}</h3>
              <p className="text-gray-500 font-normal text-sm flex-1 leading-relaxed">{reward.description}</p>
              
              <div className="mt-8 pt-6 border-t border-gray-50 flex items-center justify-between">
                <span className="font-medium text-gray-700">{reward.points.toLocaleString()} <span className="text-sm text-gray-400">pts</span></span>
                {canAfford ? (
                  <span className="text-xs uppercase tracking-widest text-gray-400 font-medium group-hover:text-gray-700 transition-colors">Redeem</span>
                ) : (
