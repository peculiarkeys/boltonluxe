...
const SignUp = () => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Automatically create the loyalty profile and guest profile if they don't exist
      if (data?.user?.email) {
        // Generate a random member ID for the new user
  ...
  return (
    <div className="h-screen w-full flex flex-row-reverse font-normal bg-white">
        ...
        <div className="w-full lg:w-1/2 flex flex-col justify-center px-8 sm:px-16 lg:px-24 overflow-y-auto">
          <div className="w-full max-w-md mx-auto">
            <h1 className="text-4xl font-medium text-gray-900 mb-2 tracking-tight">Create Account</h1>
            ...
            <p className="mt-8 text-center text-gray-500 text-sm">
              Already have an account? <Link to="/login" className="text-gray-900 font-medium ml-1 relative inline-block after:content-[''] after:absolute after:w-full after:h-[2px] after:bg-gray-900 after:left-0 after:-bottom-1">Sign in</Link>
            </p>
          </div>
        </div>
